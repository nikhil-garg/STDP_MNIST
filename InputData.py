...
class PresentInputWithPauseAsSpikes(Process):
    """Present a series of inputs as spikes, each for the same fixed length of time.
    ...
    def make_step(self, shape_in, shape_out, dt, rng, state):
        ...
        def toSpike(Input,ti):
            spikes = np.zeros_like(Input)
            step = int(ti / dt)
            spikes[...] = (Input * 22)
            spikes = spikes.astype(int)
            np.putmask(spikes,spikes < step,0)
            np.putmask(spikes,(spikes >= step) & (spikes != 0),1)
            return spikes
        ...
        def step_presentinput(t):
            ...
            i = int(t / total_time)
            ti = t % total_time
            return np.zeros_like(inputs[0]) if ti > presentation_time else toSpike(inputs[i % n],ti)
