...
class SimSTDP(Operator):
    ...
    def make_step(self, signals, dt, rng):
        ...
        def step_stdp():
            ...
            #pre_trace[pre_activities > 0] = round(self.t,3)
            #post_trace[post_activities > 0] = round(self.t,3)
            ...
            np.putmask(delta,((post_activities[:, np.newaxis] > 0) & (pre_trace[np.newaxis, :] >= (post_trace[:, np.newaxis] - self.dtSTDP))),alphaP * np.exp( - self.beta_p * (( weights - self.min_weight )/( self.max_weight - self.min_weight )))) #alphaP * weights * (1 - weights)

            np.putmask(delta,((post_activities[:, np.newaxis] > 0) & (pre_trace[np.newaxis, :] < (post_trace[:, np.newaxis] - self.dtSTDP))),- alphaN * np.exp( - self.beta_n * (( self.max_weight - weights )/( self.max_weight - self.min_weight )))) #- alphaN * weights * (1 - weights)

            #np.putmask(delta,((post_activities[:, np.newaxis] > 0) & (post_trace[:, np.newaxis] - pre_trace[np.newaxis, :] <= self.dtSTDP)),alphaP * np.exp( - self.beta_p * (( weights - self.min_weight )/( self.max_weight - self.min_weight ))))

            #np.putmask(delta,((post_activities[:, np.newaxis] > 0) & (post_trace[:, np.newaxis] - pre_trace[np.newaxis, :] > self.dtSTDP)),- alphaN * np.exp( - self.beta_n * (( self.max_weight - weights )/( self.max_weight - self.min_weight ))))
