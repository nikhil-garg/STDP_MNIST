...
class LIF(AdaptiveLIF):
    probeable = ('spikes', 'voltage', 'refractory_time','adaptation','lastSpike','inhib')
    
    def __init__(self,spiking_threshold = 1, **lif_args):
    ...
    def step(self, dt, J, output, voltage, refractory_time, adaptation, lastSpike, inhib):#inhib
        ...
        #voltage = (voltage * np.exp(-(self.T-lastSpike)/0.1)) + J
        #np.putmask(voltage,inhib == 0,(voltage * np.exp(-(self.T-lastSpike)/0.1)) + J)
        ...
        # 0.001 / Dt = output one spike value 
        if(np.sum(output) > 0):
            if(np.sum(output) > (0.001 / dt)):
                mask = voltage != np.max(voltage)
            else:
                mask = output == 0
            voltage[mask] = 0 
            J[mask] = 0 
        ...
        #t_spike = dt + tau_rc * np.log1p( -(voltage[spiked_mask] - self.spiking_threshold) / (J[spiked_mask] - self.spiking_threshold))
        ...
        #print(np.max(adaptation))
        #input("***")
        #AdaptiveLIF.step(self, dt, J, output, voltage, refractory_time, adaptation)
        inhib[inhib != 0] += - 1
        lastSpike[output>0] = self.T
        #print("\n",self.T,voltage,inhib,"\n")
        #J[...] = 0
        #output[...] = 0
...
def build_LIF(model, LIF, neurons):
    ...
    model.sig[neurons]['lastSpike'] = Signal(
        np.zeros(neurons.size_in), name="%s.lastSpike" % neurons)
    ...
    model.add_op(SimNeurons(neurons=LIF,
                            ...
                            state={"voltage": model.sig[neurons]['voltage'],
                                    ...
                                    "lastSpike": model.sig[neurons]['lastSpike'],
